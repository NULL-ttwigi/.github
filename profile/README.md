<div align="center">

# 🚀 코딩테스트 스터디, 널뛰기

![Version](https://img.shields.io/badge/Version-1.0-orange?style=flat-square)
![Author](https://img.shields.io/badge/Author-안예린-purple?style=flat-square)
[![GitHub](https://img.shields.io/badge/작성자-@girlwcode-181717?style=flat-square&logo=github)](https://github.com/girlwcode)

Welcome! 코딩테스트 준비를 위한 스터디, **널뛰기(NULL-ttwigi)** 의 레포지토리입니다. 

</div>



## 🤔 Why 널뛰기(NULL-ttwigi)?

<div align="center">

💭 **널뛰기**는 데이터 사이언티스트/분석가들이 자주 마주치는 **NULL 값**에서 영감을 받은 스터디 이름입니다.

<table style="border: none;">
<tr>
<td align="center" width="500" style="border: none;">

### 🎯 이름의 의미
- **NULL**: 데이터 분석에서 '결측치'를 의미하는 값
- **뛰기**: 끊임없이 도전하고 성장하는 모습  
- **NULL-ttwigi**: NULL 값을 뛰어넘어 확실한 데이터 처리 능력을 갖춘 주니어가 되자는 의미

</td>
<td align="center" width="500" style="border: none;">

### 💡 스터디 철학
- **결측치 처리 마스터**: NULL 값을 확실하게 처리하는 능력 기르기
- **함께 성장**: 혼자가 아닌 팀워크로 데이터 분석 역량 향상
- **실무 준비**: 코딩테스트와 데이터 분석 실력을 겸비한 전문가 되기

</td>
</tr>
</table>
</div>

<div align="center">
  
### 🚀 우리의 목표
**"NULL에서 시작해서 SUCCESS로 도약하는 데이터 사이언티스트/분석가 되기!"**

</div>

## 🎯 스터디 규칙

### 📅 진행 방식
- **주 3회** 문제 풀이
- **주말** 복습 및 추가 문제 풀이
- 매주 **리뷰 세션** 진행

### 📝 커밋 규칙(Convention)

#### 📋 기본 형식
```
[플랫폼 태그] 문제명
```
#### 🏷️ 플랫폼별 태그 규칙

| 플랫폼 | 태그 형식 | 예시 |
|--------|----------|------|
| **🥇 백준 (BOJ)** | `[BOJ-문제번호]` | `[BOJ-1000] A+B` |
| **💻 프로그래머스** | `[Programmers]` | `[Programmers] 두 정수 사이의 합` |
| **🌐 리트코드** | `[LC-문제번호]` | `[LC-1] Two Sum` |
| **🏆 SWEA** | `[SWEA]` | `[SWEA] D2 시각` |

> **⚠️ 중요**: **문제 1 개당 1 커밋**을 해주세요! 여러 문제를 한 번에 커밋하면 통계에서 정확히 반영되지 않습니다.

### 📁 폴더 구조
```
├── BOJ/          # 백준 온라인 저지
├── Programmers/  # 프로그래머스
├── LeetCode/     # 리트코드
├── SWEA/         # 삼성 SW Expert Academy
└── docs/         # 학습 자료 및 정리
```

## 🛠️ 사용 언어

<div align="center">

| 언어 | 활용 분야 |
|---------|-------------|
| **🐍 Python** | 알고리즘 구현, 데이터 처리 |
| **🗄️ SQL** | 데이터 분석, 쿼리 최적화 |

</div>

## 👥 멤버 소개

<div align="center">
<table style="border: none;">
  <tr>
    <td align="center" style="border: none;">
      <a href="https://github.com/girlwcode">
        <img src="https://avatars.githubusercontent.com/girlwcode" width="150px;" alt="안예린" style="border-radius: 50%;"/>
        <br />
        <b>🌟 안예린</b>
      </a>
      <br />
      <i>Yelyn Ahn</i>
    </td>
    <td align="center" style="border: none;">
      <a href="https://github.com/heheelee">
        <img src="https://avatars.githubusercontent.com/heheelee" width="150px;" alt="이현희" style="border-radius: 50%;"/>
        <br />
        <b>🌟 이현희</b>
      </a>
      <br />
      <i>Hyunhee Lee</i>
    </td>
    <td align="center" style="border: none;">
      <a href="https://github.com/mini-u">
        <img src="https://avatars.githubusercontent.com/mini-u" width="150px;" alt="유성민" style="border-radius: 50%;"/>
        <br />
        <b>🌟 유성민</b>
      </a>
      <br />
      <i>Sungmin Ryu</i>
    </td>
  </tr>
</table>

### 📊 멤버 정보 요약
| 이름 | 주요 언어 | 목표 | 프로필 |
|------|-----------|------|--------|
| **안예린** | **Python** | *단순히 구현만이 아니라 효율성까지!* | [💜](https://www.linkedin.com/in/yelyn-ahn/) |
| **이현희** | **SQL** | *SQL마스터하고 2025 환승이직 성공.* | [💙](https://kr.linkedin.com/in/hhlee0521) |
| **유성민** | **SQL** | *SQL마스터 후 2025 덴마크 외노자되기.* | [💚](https://www.linkedin.com/in/minnieryu/) |

</div>

<div align="center">

### 📈 멤버별 성과
| 이름 | 🎯 해결 문제 | 📅 주 목표 달성 | 🏆 최고 티어 | 📁 개인 저장소 |
|------|-------------|---------------|-------------|------------------|
| 안예린 | 0개 | 0주 | - | 1개 |
| 이현희 | 0개 | 0주 | - | 0개 |
| 유성민 | 2개 | 0주 | - | 1개 |

</div>

> 💡 **자동 업데이트**: 이 통계는 GitHub Actions를 통해 매주 자동으로 업데이트됩니다!

> 🏆 **최고 티어**: 백준/프로그래머스 등에서 달성한 최고 티어를 update_stats.py 파일에서 수동으로 업데이트해주세요!

> 🌐 **집계 범위**: 조직 내에서 해당 멤버가 생성한 퍼블릭 레포지토리만 집계됩니다.

<div align="center">

### 🏆 스터디 목표

| 기간 | 목표 | 상태 |
|---------|---------|---------|
| **단기** | 다함께 한 달 성공 | 🚀 시작! |
| **중기** | 골드 티어 달성 | 💪 도전! |
| **장기** | 취업 준비 완료 | ✨ 성공! |

</div>

## 📚 학습 자료

### 📖 추천 도서
- [이것이 취업을 위한 코딩테스트다](https://github.com/ndb796/python-for-coding-test)
- [파이썬 알고리즘 인터뷰](https://github.com/onlybooks/algorithm-interview)

### 🌐 온라인 플랫폼
- [백준 온라인 저지](https://www.acmicpc.net/)
- [프로그래머스](https://programmers.co.kr/)
- [리트코드](https://leetcode.com/)
- [SW Expert Academy](https://swexpertacademy.com/)

## 📊 문제 난이도별 분류

<div align="center">

| 난이도 | 레벨 | 설명 |
|-----------|---------|---------|
| 🟢 **브론즈** | `초급` | 기본 문법 및 간단한 구현 |
| 🟡 **실버** | `중급` | 자료구조, 기본 알고리즘 |
| 🟠 **골드** | `고급` | 고급 알고리즘, 최적화 |
| 🔴 **플래티넘** | `최고급` | 복잡한 알고리즘, 최적화 |

</div>


## 📝 문제 풀이 템플릿

```python
# 문제: [문제번호] 문제명
# 난이도: 브론즈/실버/골드/플래티넘
# 링크: https://www.acmicpc.net/problem/문제번호

def solution():
    # 여기에 코드 작성
    pass

if __name__ == "__main__":
    solution()
```

## 🎉 성과 기록

<div align="center">

| 항목 | 현황 |
|---------|---------|
| **총 해결 문제** | 2개 |
| **현재 최장 연속 풀이** | 0주 (🏆 안예린, 이현희, 유성민) |
| **마지막 업데이트** | 2026년 02월 16일 |

</div>


## 🤖 자동 통계 업데이트

**GitHub Actions**를 통해 멤버들의 레포지토리 커밋 통계를 **매주 일요일 자정**에 자동으로 업데이트합니다.

<div align="center">

### 📊 업데이트 항목

| 통계 종류 | 설명 |
|-------------|---------|
| **개인 레포지토리 수** | 조직 내에서 생성한 퍼블릭 레포지토리 개수 |
| **문제 풀이 커밋 수** | 커밋 메시지 패턴으로 필터링된 실제 문제 풀이 |
| **총 커밋 수** | 전체 커밋 활동 현황 |
| **주 목표 달성 주 수** | 주 3커밋 이상을 달성한 주의 개수 |

### ⚠️ 문제 풀이 필터링 패턴

| 플랫폼 | 커밋 메시지 형식 | 예시 |
|-----------|-------------------|------|
| **백준** | `[BOJ-숫자]` | `[BOJ-1000] A+B` or `[BOJ-1] A+B` |
| **프로그래머스** | `[Programmers]` | `[Programmers] 두 정수 사이의 합` |
| **리트코드** | `[LC-숫자]` | `[LC-0001] Two Sum` or `[LC-1] Two Sum` |
| **SWEA** | `[SWEA]` | `[SWEA] D2 문제` |

</div>

> 💡 **중요**: 위 패턴과 일치하는 커밋만 문제 풀이로 집계됩니다!




